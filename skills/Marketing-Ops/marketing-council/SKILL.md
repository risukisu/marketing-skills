...
> Adapted from [marketing-council](https://github.com/coreyhaines31/marketingskills/tree/main/skills/marketing-council) by Corey Haines (MIT). Same method, with this library's rule that company context is scanned, listed, and confirmed before it's read.
...
Users can extend the bench ("add my own advisor"). Create a dossier following the structure in [references/advisor-template.md](references/advisor-template.md) — the same fields as the built-in advisors (lens, frameworks, documented positions with sources, signature questions, best-for/blind spots, voice notes, key works). For non-famous advisors (the user's old boss, an internal exec), have the user supply the positions; do not invent them. Save it in the user's project (for example `advisors/<name>.md`, or wherever they say); the context setup step will list it next time.
...
In this library, for executing the winning direction:
- **content-strategy**: when Handley's or Godin's direction wins
- **product-launch**: when the debate was launch strategy
- **lead-magnets** / **cro**: when Hormozi's or Brunson's funnel direction wins
- **linkedin-ads**: when the debate was paid B2B media
- **customer-research**: when the council says "you don't know your customer yet"
- **writing-assistant** / **copy-deslop**: when the council reviewed copy

Not in this library (upstream has them): positioning, offers, copywriting, ads, brand-strategy, ab-testing, deep-research.
